...
describe('inform', () => {
  ...
  it('lists submissions for a form', async () => {
    const { app } = await createSystemServer(inform, config, { logLevel: 'silent' });
    const res = await app.inject({ method: 'GET', url: '/data/6225' });
    expect(res.statusCode).toBe(200);
    const body = res.json();
    expect(Array.isArray(body.results)).toBe(true);
    expect(body.results.length).toBeGreaterThan(0);
  ...
  it('fetches a single submission by id', async () => {
    const { app } = await createSystemServer(inform, config, { logLevel: 'silent' });
    const res = await app.inject({ method: 'GET', url: '/data/6225/7783155' });
    expect(res.statusCode).toBe(200);
  ...
  it('404s for an unknown attachment', async () => {
    const { app } = await createSystemServer(inform, config, { logLevel: 'silent' });
    const res = await app.inject({ method: 'GET', url: '/media/does-not-exist' });
  ...
  it('fetches attachment metadata', async () => {
    const { app } = await createSystemServer(inform, config, { logLevel: 'silent' });
    const res = await app.inject({ method: 'GET', url: '/media/621985' });
    expect(res.statusCode).toBe(200);
