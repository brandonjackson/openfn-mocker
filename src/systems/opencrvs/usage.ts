...
export const usage: UsageExample[] = [
  ...
  { fn: "http.post", signature: "http.post(path, body, options)", description: "Make a raw POST request to any OpenCRVS events REST endpoint. Events are listed through the search endpoint — there is no GET collection route.",
    code: "http.post('/api/events/events/search', {\n  query: { type: 'and', clauses: [{ eventType: 'v2.birth' }] },\n});", apiRef: "ex1" },
  { fn: "http.request", signature: "http.request(method, path, body, options?)", description: "Make a raw HTTP request with any method to an OpenCRVS events REST endpoint.",
    code: "http.request('GET', '/api/events/events/2e5c9a71-4b3d-4e82-a9f6-0c7d1b8e35a4');", apiRef: "ex4" },
];
